...
async def omdb_request(params: Dict[str, Any], allow_not_found: bool = False):
    query = dict(params)
...
async def get_omdb_details(title: str):
    ...
    data = await omdb_request({"t": title, "plot": "full"}, allow_not_found=True)
    if data:
...
async def enrich_cards(rows) -> List[MovieCard]:
    ...
    # rows comes from DataFrame.itertuples(), so each row is a namedtuple.
...
def tfidf_recommend_titles(query_title: str, top_n: int = 10) -> List[Tuple[str, float]]:
    ...
    for i in order:
        ...
        if not title.strip():
            continue
        recommendations.append((title, float(scores[int(i)])))
...
async def home(
    ...
    if category in {"popular", "trending"}:
        ...
        data = data[dates > pd.Timestamp.today()]
    else:
        raise HTTPException(status_code=400, detail="Invalid category.")

    data = data[data["title"].str.strip() != ""].head(limit)
    return await enrich_cards(data.itertuples(index=False))
...
async def movie_search(query: str = Query(..., min_length=1)):
    ...
    for item in data.get("Search", []):
        ...
        local_id = int(local_movie["id"]) if local_movie is not None else 0
...
async def movie_details(movie_id: int):
    ...
    if not data:
        raise HTTPException(status_code=404, detail="Movie details not found in OMDb.")
...
async def recommend_genre(
    ...
    mask = movies["genres"].apply(lambda x: target_genre in get_genre_names(x))
    recommendations = movies[mask & (movies["id"] != movie_id)]
    recommendations = recommendations.sort_values("popularity", ascending=False).head(limit)
    return await enrich_cards(recommendations.itertuples(index=False))
...
async def search_bundle(
    ...
    tfidf_recs = tfidf_recommend_titles(details.title, tfidf_top_n)
    tfidf_items = []
    ...
    for title, score in tfidf_recs:
        ...
        if local is not None:
            ...
            card = MovieCard(
                ...
                poster_url=poster,
                release_date=str(local.get("release_date", "")) or None,
                vote_average=safe_float(local.get("vote_average")),
            )
